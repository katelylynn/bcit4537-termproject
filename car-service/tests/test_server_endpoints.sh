...
curl -X POST -H "Content-Type: application/json" -d '{"speed": 0.8, "harshness": 0.2}' "$BASE_URL/forward"
echo -e "\n"

# Test backward endpoint
echo "Testing /backward endpoint..."
curl -X POST -H "Content-Type: application/json" -d '{"speed": 0.6, "harshness": -0.3}' "$BASE_URL/backward"
echo -e "\n"

# Test rotate left endpoint
echo "Testing /rotate endpoint with direction left..."
curl -X POST -H "Content-Type: application/json" -d '{"direction": "left", "speed": 0.7}' "$BASE_URL/rotate"
echo -e "\n"

# Test rotate right endpoint
echo "Testing /rotate endpoint with direction right..."
curl -X POST -H "Content-Type: application/json" -d '{"direction": "right", "speed": 0.7}' "$BASE_URL/rotate"
echo -e "\n"
